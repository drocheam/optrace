...
# make Raytracer
RT = Raytracer(outline=[-4, 5, -3, 5, -1, 27.5])
...
# prism surface
P1 = SurfaceFunction(r=3, func=lambda x, y: 0.5*x, derivative=lambda x, y: (np.full_like(x, 0.5), 0*y))
P2 = SurfaceFunction(r=3, func=lambda x, y: -0.5*x, derivative=lambda x, y: (np.full_like(x, -0.5), 0*y))
...
# add Detector
Det = Detector(Surface("Rectangle", dim=[5, 5]), pos=[0, 0, 23.])
RT.add(Det)
