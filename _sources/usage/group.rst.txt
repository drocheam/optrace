...
A |Group| is a container for several geometry elements.
...
.. list-table::
   ...
   * - Rotate or flip the whole group: 
     - :python:`G.rotate(-12),`
       :python:`G.flip()`
...
Here, |IdealLens| and |Lens| are included in the same lens list, marker types are included in :python:`markers` 
and all volume types in :python:`volumes`.

When adding objects, the ordering of former objects inside the list remains the same.
Thus :python:`lenses[2]` denotes the lens that was added third (counting starts at 0).
For clarity, it is recommended to add objects in their correct z-order along the optical axis.

Geometry properties
###########################

A group shares many functions for geometry properties and manipulations with the |Element| classes, 
see :ref:`element_geometry_props`. There are three main differences:

**Position**

The position property describes the position of the first element (smallest z-position) in the group.

**Flipping**

When flipping, optional parameters :python:`y0`, :python:`z0` define a rotation axis.
By default, :python:`y0 = 0` and :python:`z0` are the center of the extent of the group.
See :func:`Group.flip <optrace.tracer.geometry.group.Group.flip>` for more details.

**Rotation**

For rotation, parameters :python:`x0`, :python:`y0` can be provided, 
which also describe a position of the rotation axis.
They default to zero, see :func:`Group.rotate <optrace.tracer.geometry.group.Group.rotate>`.
...
(which after flipping is the filter), whereas all relative distances between elements are kept equal.
...
optrace supports the import of basic ``.zmx`` geometries into the raytracer. 
The following example loads a geometry from file ``setup.zmx``:
...
For the materials to be loaded correctly, all required materials inside the ``.zmx``
need to be included with the correct naming in the :python:`n_dict` dictionary.
You can either load them from a ``.agf`` catalogue like in :numref:`agf_load` or create the dictionary manually.

Exemplary ``.zmx`` files can be found in the following 
`repository <https://github.com/nzhagen/LensLibrary/tree/main/zemax_files>`_.

Unfortunately, the support is only limited, as there is no official documentation on the file format.
Additionally, only a subset of all ZEMAX OpticStudio functionality is supported, including:

* sequential simulation (``SEQ``-mode only)
* units in millimeters (``UNIT`` must be ``MM``)
* only ``STANDARD`` or ``EVENASPH`` surfaces, this allows for 
...
* only loads lens and aperture geometries, no support for any additional objects

Some information on the file format are available in :footcite:`Zemax_2000`, 
...
An ideal camera preset provides aberration-free imaging towards a detector.
...
Required parameters are the object position :python:`z_g` , the camera position (the position of the lens) 
:python:`cam_pos`, and the image distance :python:`b`, 
...
Additional lens diameter parameter :python:`r` and detector radius :python:`r_det` parameters are available:
...
When given a desired object magnification :math:`m`, the image distance parameter :math:`b` is calculated according to:

.. math::

   m = \frac{b}{g} \Rightarrow b = m \, g

Here, :math:`g` is the object distance, in our example :python:`z_g - cam_pos[2]`.
...
The LeGrand full theoretical eye is a simple eye model consisting of spherical surfaces only
...
:mod:`ot.presets.geometry <optrace.tracer.presets.geometry>`. 
The function returns a |Group| object that can be added to a |Raytracer|. 
Provide a :python:`pos` parameter to position it at any other location than :python:`[0, 0, 0]`.
...
Optional parameters include a pupil diameter and a lateral detector (retina) radius, both given in millimeters.
...
It consists of conic surfaces, dispersive media and an adaptation dependent geometry.
...
With an accommodation level :math:`A` in dpt, the parameters 
are calculated as: :footcite:`SchwiegerlingOptics`
...
Accessing and adding the group works is handled the same way as for the 
...
As for the :func:`legrand_eye <optrace.tracer.presets.geometry.legrand_eye>`, the parameters 
:python:`pupil` and :python:`r_det` are available. 
There is an additional :python:`adaptation` parameter, specified in diopters, which defaults to 0 dpt.
