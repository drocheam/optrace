...
from optrace.tracer.geometry.surface_function import SurfaceFunction  # user surface functions
...
class Surface(BaseClass):
    ...
    def __init__(self,
        ...
        match surface_type:
            ...
            case ("Asphere" | "Sphere"):
                ...
                z1 = self.get_values(np.array([r + self.pos[0]]), np.array([self.pos[1]]))[0]
            ...
            case "Data":
                ...
                if not np.all(np.isfinite(Z)):
                    raise RuntimeError("Non-finite data in 'Data' surface matrix")
                ...
                self.eps = (xy[1] - xy[0]) / 1e3

                # remove offset at center
                Z -= misc.interp2d(xy, xy, Z, np.array([0.]), np.array([0.]))
               
                # biquadratic interpolation, otherwise we would have discrete glass planes
                self._interp = scipy.interpolate.RectBivariateSpline(xy, xy, Z, kx=2, ky=2)
                
                # provisional zmin, zmax, so get_edge() works
                self.zmin, self.zmax = np.nanmin(Z), np.nanmax(Z)
                
                # get surface mask
                X, Y = np.meshgrid(xy, xy)
                Mask = self.get_mask(X.ravel(), Y.ravel()).reshape(X.shape)
                # get numeric data as well as interpolated circular edge
                # since min/max values could be interpolated values at edge
                hdata = np.concatenate((Z[Mask].ravel(), self.get_edge(10000)[2]))
                self.zmin, self.zmax = np.min(hdata), np.max(hdata)  # find new zmin and zmax
    ...
    def has_hit_finding(self) -> bool:
        ...
        return self.surface_type in ["Circle", "Ring", "Rectangle", "Sphere", "Asphere"] \
            or (self.surface_type == "Function" and self.func.has_hits())
    ...
    def is_2d(self) -> bool:
        """:return: if the surface is actual 2D and no Point or Line"""
    ...
    def get_extent(self) -> tuple[float, float, float, float, float, float]:
        """
    ...
    def get_values(self, x: np.ndarray, y: np.ndarray) -> np.ndarray:
        ...
        """
        if self.is_planar():
            return np.full_like(x, self.pos[2], dtype=np.float64)
            ...
            if np.count_nonzero(inside):
                ...
                match self.surface_type:
                    ...
                    case ("Asphere" | "Sphere"):
                        r2i, k, rho = r2[inside], self.k, self.rho
                    ...
                    case "Data":
                        ...
                        z[inside] = self.pos[2] + self._interp(x[inside]-self.pos[0], y[inside]-self.pos[1], grid=False)

                    case "Function":
                        # additional mask function is handled in get_values()
                        z[inside] = self.pos[2] + self.func.get_values(x[inside] - self.pos[0], y[inside] - self.pos[1])
    ...
    def get_plotting_mesh(self, N: int) -> tuple[np.ndarray, np.ndarray, np.ndarray]:
        ...
        if self.surface_type == "Rectangle":
            xs, xe, ys, ye, _, _ = self.get_extent()
        ...
        X, Y = np.meshgrid(x, y)
        z = self.get_values(X.ravel(), Y.ravel())
        ...
        z[mask] = self.get_values(self.pos[0] + r * np.cos(Phi.ravel()[mask]),
                                  self.pos[1] + r * np.sin(Phi.ravel()[mask]))
        ...
        mask3 = self.get_mask(X.ravel(), Y.ravel())
        z[~mask & ~mask3] = np.nan
    ...
    def get_mask(self, x: np.ndarray, y: np.ndarray) -> np.ndarray:
        ...
        match self.surface_type:
            ...
            case "Function":
                return self.func.get_mask(x - self.pos[0], y - self.pos[1])

            case ("Circle" | "Ring" | "Sphere" | "Asphere"| "Data"):
    ...
    def get_normals(self, x: np.ndarray, y: np.ndarray) -> np.ndarray:
        ...
        match self.surface_type:
            ...
            case "Function" if self.func.has_derivative():
                n = np.full((x.shape[0], 3), -1, dtype=np.float64, order='F')

                n[:, 0], n[:, 1] = self.func.get_derivative(x - self.pos[0], y - self.pos[1])

                return -misc.normalize(n)
            ...
            case _:
                ...
                ds = self.eps
                n = np.zeros((x.shape[0], 3), dtype=np.float64, order='F')
                n[:, 0] = self.get_values(x - ds / 2, y) - self.get_values(x + ds / 2, y)  # -uz
                n[:, 1] = self.get_values(x, y - ds / 2) - self.get_values(x, y + ds / 2)  # -vz
                n[:, 2] = ds
    ...
    def find_hit(self, p: np.ndarray, s: np.ndarray) -> tuple[np.ndarray, np.ndarray]:
        ...
        match self.surface_type:
            ...
            case "Function" if self.func.has_hits():

                p_hit0 = self.func.get_hits(p - self.pos, s)
            ...
            case ("Sphere" | "Asphere"):
                ...
                t = ne.evaluate("where((zmin <= z1) & (z1 <= zmax), t1, t2)")
                ...
                # simplest case: ray shooting straight at center of a parabola with 1/rho = 0,
    ...
    def __setattr__(self, key, val):
        ...
        match key:
            ...
            case "surface_type":
                ...
                pc.checkIfIn(key, val, self.surface_types)
            ...
            case "func":
                pc.checkType(key, val, SurfaceFunction | None)

            case "dim":
                pc.checkType(key, val, np.ndarray)
