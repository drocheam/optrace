...
from optrace.tracer.refraction_index import RefractionIndex  # material refraction index
...
class Lens(SObject):
    ...
    def estimate_focal_length(self,
        ...
        def getR(Surf):
            if Surf.surface_type in ["Sphere", "Asphere"]:
                ...
                raise RuntimeError("Calculation only possible with surface_type 'Sphere', "
                                   "'Asphere' or planar surfaces.")
    ...
    def __setattr__(self, key, val):
        ...
        if key == "n":
            pc.checkType(key, val, RefractionIndex)
